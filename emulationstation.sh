  ...
  pocet=0    
  {
    while IFS= read -r line; do
      if [[ ! ${line} =~ \[BIOS\] ]]; then
        ...
        echo "<figure><a href=\"#\" onclick=\"window.location.href='https://${location}/${rom[1]}/${ahref}'\"><img title=\"${line%.*}\" loading=lazy src=\"https://thumbnails.libretro.com/${rom[2]}/Named_Snaps/${line%.*}.png\" onerror=\"this.src='https://thumbnails.libretro.com/${rom[2]}/Named_Snaps/${thumb%.*}.png';this.onerror=''\"><figcaption>${line%.*}</figcaption></a></figure>" >> ~/online/${rom[0]}.html
        ((pocet++))
        ((total++))
      fi
    done
