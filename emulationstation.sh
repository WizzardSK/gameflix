  ...
  pocet=0    
  {
    while IFS= read -r line; do
      if [[ ! ${line} =~ \[BIOS\] ]]; then
        ...
        echo "<figure><a href=\"https://${location}/${rom[1]}/${line}\"><img title=\"${line%.*}\" loading=lazy src=\"https://thumbnails.libretro.com/${rom[2]}/Named_Snaps/${line%.*}.png\" onerror=\"this.src='https://thumbnails.libretro.com/${rom[2]}/Named_Snaps/${thumb%.*}.png';this.onerror=''\"><figcaption>${line%.*}</figcaption></a></figure>" >> ~/online/${rom[0]}.html
        ((pocet++))
        ((total++))
      fi
    done
  ...
  pocet=0
  {
    while IFS= read -r line; do
      if [[ ! ${line} =~ \[BIOS\] ]]; then
        ...
        echo "<figure><a href=\"https://myrient.erista.me/files/${zip[1]}\"><img title=\"${line%.*}\" loading=lazy src=\"https://thumbnails.libretro.com/${zip[2]}/Named_Snaps/${line%.*}.png\" onerror=\"this.src='https://thumbnails.libretro.com/${zip[2]}/Named_Snaps/${thumb%.*}.png';this.onerror=''\"><figcaption>${line%.*}</figcaption></a></figure>" >> ~/online/${zip[0]}.html
        ((pocet++))
        ((total++))
      fi
    done
