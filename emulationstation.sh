  ...
  pocet=0    
  {
    while IFS= read -r line; do
      if [[ ! ${line} =~ (\[BIOS\]|\(Beta|\(Demo\)|\(Aftermarket\)|\(Proto|\(Unl\)|\(Program\)|\(Alt\)) ]]; then
        ...
        echo "<figure><a href=\"roms/${rom[0]}/${line}\"><img title=\"${line%.*}\" src=\"http://thumbnails.libretro.com/${rom[2]}/Named_Snaps/${line%.*}.png\"><figcaption>${line%.*}</figcaption></a></figure>" >> ~/${rom[0]}.html
        ((pocet++))
        ((total++))
      fi
    done
  ...
  pocet=0
  {
    while IFS= read -r line; do
      if [[ ! ${line} =~ (\[BIOS\]|\(Beta|\(Demo\)|\(Aftermarket\)|\(Proto|\(Unl\)|\(Program\)|\(Alt\)) ]]; then
        ...
        echo "<figure><a href=\"roms/${zip[0]}/${line}\"><img title=\"${line%.*}\" src=\"http://thumbnails.libretro.com/${zip[2]}/Named_Snaps/${line%.*}.png\"><figcaption>${line%.*}</figcaption></a></figure>" >> ~/${zip[0]}.html
        ((pocet++))
        ((total++))
      fi
    done
