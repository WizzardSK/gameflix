#!/bin/bash
params="--config=/userdata/system/.config/rclone/rclone.log --no-checksum --no-modtime --attr-timeout 100h --dir-cache-time 100h --poll-interval 100h --vfs-cache-mode full --allow-non-empty --daemon"
