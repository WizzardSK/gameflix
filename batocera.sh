#!/bin/bash
params="--log-file=/userdata/system/rclone.log --config=/userdata/system/.config/rclone/rclone.log --no-checksum --no-modtime --attr-timeout 100h --dir-cache-time 100h --poll-interval 100h --vfs-cache-mode full --allow-non-empty --daemon"
