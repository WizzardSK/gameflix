...
rclone mount "archive:retroarch-bios" /recalbox/share/bios --log-file=/mylogfile.txt --config=/recalbox/share/system/.config/rclone/rclone.conf --no-checksum --no-modtime --attr-timeout 100h --dir-cache-time 100h --poll-interval 100h --vfs-cache-mode full --allow-non-empty --daemon
IFS=","
for each in "${roms[@]}"
do
