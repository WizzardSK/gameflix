...
roms+=("n64,myrient:No-Intro/Nintendo - Nintendo 64 (BigEndian),Nintendo - Nintendo 64/Named_Snaps")
...
do
  ...
  echo "</gameList>" >> /recalbox/share/roms/${rom[0]}/gamelist.xml
  rclone mount thumbnails:${rom[2]} /recalbox/share/roms/${rom[0]}/media --config=/recalbox/share/system/.config/rclone/rclone.conf --daemon --vfs-cache-mode full --no-checksum --no-modtime --attr-timeout 100h --dir-cache-time 100h --poll-interval 100h --allow-non-empty
done
