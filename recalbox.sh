...
do
  ...
  rclone mount ${rom[1]} /recalbox/share/roms/${rom[0]}/online -vv --log-file /tmp/rclone.log --file-perms 777 --config=/recalbox/share/system/.config/rclone/rclone.conf --daemon --vfs-cache-mode full --no-checksum --no-modtime --attr-timeout 100h --dir-cache-time 100h --poll-interval 100h --allow-non-empty --allow-other
