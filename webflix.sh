...
while IFS=';' read -ra rom; do
  path="${rom[1]}"
  [[ "$path" =~ ^archive:([^/]+)/(.+) ]] && remote="${BASH_REMATCH[1]}" subpath="/${BASH_REMATCH[2]}"
  [[ -n "$remote" ]] || continue
  archive_path=$(dirname "$subpath")
  already_mounted=false
  for m in "${mounted[@]}"; do [[ "$m" == "$remote" ]] && already_mounted=true && break; done
  if [[ "$already_mounted" == "false" ]]; then
    mounted+=("$remote")
    mkdir -p ~/zips/$remote
    mountpoint -q ~/zips/$remote || nohup rclone mount "archive:$remote" ~/zips/$remote --no-checksum --no-modtime --attr-timeout 1000h --dir-cache-time 1000h --allow-non-empty --allow-other --vfs-cache-mode minimal --vfs-read-chunk-size 1M > /dev/null 2>&1 &
...
while IFS=';' read -ra rom; do
  ...
  bindfs --perms=0755 --force-user=$(whoami) --force-group=$(id -gn) ~/zips/$remote$subpath ~/roms/$platform/$display
done <<< "$csv"
