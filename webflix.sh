...
mkdir -p ~/myrient ~/roms/{TIC-80,LowresNX,WASM-4,DICE} ~/iso ~/zips ~/gameflix
wget -nv -O ~/.config/rclone/rclone.conf https://raw.githubusercontent.com/WizzardSK/gameflix/main/rclone.conf
archives=(https://wizzardsk.github.io/{lowresnx,tic80,wasm4,dice}.zip)
...
bindfs --perms=0755 --force-user=$(whoami) --force-group=$(id -gn) $HOME/zips/dice.zip "$HOME/roms/DICE"
