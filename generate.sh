...
echo "TIC-80"; cp platform.html ~/gameflix/TIC-80.html; echo "<script>bgImage(\"tic80\"); var fileNames = [" >> ~/gameflix/TIC-80.html; ((platforms++))
...
echo "LowresNX"; cp platform.html ~/gameflix/LowresNX.html; echo "<script>bgImage(\"lowresnx\"); var fileNames = [" >> ~/gameflix/LowresNX.html; ((platforms++))
...
echo "WASM-4"; cp platform.html ~/gameflix/WASM-4.html; echo "<script>bgImage(\"wasm4\"); var fileNames = [" >> ~/gameflix/WASM-4.html; ((platforms++))
...
echo "PICO-8"; cp platform.html ~/gameflix/PICO-8.html; echo "<script>bgImage(\"pico8\"); var fileNames = [" >> ~/gameflix/PICO-8.html; ((platforms++))
...
echo "Voxatron"; cp platform.html ~/gameflix/Voxatron.html; echo "<script>bgImage(\"voxatron\"); var fileNames = [" >> ~/gameflix/Voxatron.html; ((platforms++))
...
IFS=";"; for each in "${roms[@]}"; do
  ...
  if [[ "$rom3" != "${rom[0]}" ]]; then
    ...
    echo "var fileNames = [" >> ~/gameflix/${rom[0]}.html;
    pocet=0; 
  fi
