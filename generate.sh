...
curl -s "https://raw.githubusercontent.com/WizzardSK/gameflix/refs/heads/main/lowresnx.txt" | while IFS="|" read -r id name picture cart; do if [[ -n "$cart" && -n "$picture" ]]; then echo "\"$cart,$picture\"," >> ~/gameflix/LowresNX.html; fi; done
