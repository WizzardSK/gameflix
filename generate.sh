...
echo "<gameList>" > ~/gamelists/switch/gamelist.xml; cat switch.txt | while read -r name; do echo "<game><path>./${name}</path><name>${name}</name><image>./${picture}</image></game>" >> ~/gamelists/switch/gamelist.xml
