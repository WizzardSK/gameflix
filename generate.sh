...
while IFS= read -r path; do
  ...
  (
    if [[ "$path" == https://* ]]; then
      ...
      unzip -l "$tmp" 2>/dev/null | awk '$2 ~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/ && $3 ~ /^[0-9]{2}:[0-9]{2}$/ {match($0,/^ *[0-9]+ +[0-9-]+ +[0-9:]+ +/); name=substr($0,RLENGTH+1); sub(/^[a-z0-9_]+\//, "", name); print name}' > "$cache/$h.txt"
      rm -f "$tmp"
    elif [[ "$path" == *:* ]]; then
      aftercolon="${path#*:}"
