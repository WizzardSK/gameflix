...
while IFS= read -r path; do
  ...
  (
    if [[ "$path" == https://* ]]; then
      ...
      unzip -l "$tmp" 2>/dev/null | awk '$2 ~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/ && $3 ~ /^[0-9]{2}:[0-9]{2}$/ {match($0,/^ *[0-9]+ +[0-9-]+ +[0-9:]+ +/); print substr($0,RLENGTH+1)}' > "$cache/$h.txt"
      rm -f "$tmp"
    elif [[ "$path" == *:* ]]; then
      aftercolon="${path#*:}"
      localpath="$HOME/mount/$aftercolon"
      if [[ "$localpath" == *.zip ]]; then
        unzip -l "$localpath" 2>/dev/null | awk '$2 ~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/ && $3 ~ /^[0-9]{2}:[0-9]{2}$/ {match($0,/^ *[0-9]+ +[0-9-]+ +[0-9:]+ +/); print substr($0,RLENGTH+1)}' > "$cache/$h.txt"
      else
        zipcount=$(find "$localpath" -name "*.zip" 2>/dev/null | wc -l)
        if [ "$zipcount" -ge 1 ]; then
          zipfile=$(find "$localpath" -name "*.zip" 2>/dev/null | head -1)
          unzip -l "$zipfile" 2>/dev/null | awk '$2 ~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/ && $3 ~ /^[0-9]{2}:[0-9]{2}$/ {match($0,/^ *[0-9]+ +[0-9-]+ +[0-9:]+ +/); print substr($0,RLENGTH+1)}' > "$cache/$h.txt"
